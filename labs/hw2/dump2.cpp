...
disassemble(pid_t proc, unsigned long long rip, const char *module) {
	...
	for(ptr = rip; ptr < rip + sizeof(buf); ptr += PEEKSIZE) {
		...
		fprintf(stderr, "byte str = %d | peek = %llx | %llx\n", buf[ptr-rip], peek, ptr-rip);
	}
	...
	// for (pair<long long, instruction1> t : instructions) {
	// 	fprintf(stderr, "%lld %8s %d %s %s\n", t.first, string(t.second.bytes), t.second.size, t.second.opr, t.second.opnd);
	// }
